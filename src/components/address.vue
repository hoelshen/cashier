<!-- 
     ...
     *      
     *      属性：                
     *          provinceCode         省code          默认110000
     *          cityCode             市code          默认110100
     *          areaCode             区code          默认110101
     *          content              详细地址         默认 ''
     *
     *      事件：
     *          getAllData      省市区变更的时候或者详细地址失焦的时候触发，参数是一个对象，包含省市区名字以及编码等详细的信息。
     *
     *
     *      方法：
...
<template>
    <div class="zp-address">
        ...
        <!-- <div class="address-content">
            <el-input v-model="onSelect.content" placeholder="请输入内容" @blur="getAllData"></el-input>
        </div> -->
    </div>
</template>
<script>
    ...
    export default {
        ...
        computed: {
            cityList() {
                for (var attr in this.DISTRICTS[this.onSelect.provinceCode]) {
                    break;
                }
                if (this.isFirst) {
                    this.onSelect.cityCode = this.cityCode;
                } else {
                    this.onSelect.cityCode = attr;
                }
                return this.DISTRICTS[this.onSelect.provinceCode];
            },
            areaList() {
                for (var attr in this.DISTRICTS[this.onSelect.cityCode]) {
                    break;
                }
                if (this.isFirst) {
                    this.onSelect.areaCode = this.areaCode;
                } else {
                    this.onSelect.areaCode = attr;
                }
        ...
        methods: {
            getData() {
                ...
                // 详细地址
                let content = this.onSelect.content.trim();
                return {
                    provinceCode,
                    provinceName,
                    cityCode,
                    ...
                    areaName,
                    content,
                    address: String(provinceName) + String(cityName) + String(areaName) + String(content)
                }
            },
            getAllData(ev) {
                if (this.isFirst && !ev.target) {
                    this.isFirst = false;
                }
